...
class Experiment(object):
    ...
    def compute_energy(self):
        ...
        energies = []
        with torch.no_grad():
            for data, (inputs, labels) in enumerate(self.trainloader):
    ...
    def compute_bayes_loss(self, dataloader, num_sgld_iter=50, max_data_size=float('+inf')):
        rec = []
        numdata = 0
        for i, (inputs, labels) in enumerate(dataloader):
            ...
            # but why not?
            val_array = []
            for model_copy in self.run_sgld_chains(num_sgld_iter, dataloader, gamma=self.sgld_gamma):
                outputs = model_copy(inputs, labels=labels)
            ...
            rec.append(lse.item())
            numdata += 1
            if numdata > max_data_size: break
        return -np.mean(rec)

    def compute_gibbs_loss(self, dataloader, num_sgld_iter=50, max_data_size=float('+inf')):
        mean_mean_losses = 0
        num_mean_losses = 0
        for model_copy in self.run_sgld_chains(num_sgld_iter, dataloader, gamma=self.sgld_gamma):
            mean_losses = 0
            ...
            for batch, (inputs, labels) in enumerate(dataloader):
                inputs, labels = inputs.to(self.device), labels.to(self.device)
                outputs = model_copy(inputs, labels=labels)
                mean_losses += outputs.loss.item()
                del outputs
                num_data += 1
                if num_data > max_data_size: break
            mean_losses = mean_losses / num_data

            mean_mean_losses += mean_losses
            num_mean_losses += 1
        return -mean_mean_losses/num_mean_losses
